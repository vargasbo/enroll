...
RSpec.describe Insured::FamilyRelationshipsController, dbclean: :after_each do
  ...
  describe "POST create" do
    ...
    let(:family_member1) {FactoryBot.create(:family_member, family: test_family).person}
    ...
    before :each do
      ...
      @controller.instance_variable_set(:@person, person)
      @controller.instance_variable_set(:@family, test_family)
      post :create, params: {:predecessor_id => family_member1.id, :successor_id => primary_member.id, :kind => "spouse", :redirect_url => insured_family_relationships_path}
    end
    ...
    describe "with multiple family members" do
      let(:child1) {FactoryBot.create(:family_member, family: test_family).person}
      let(:child2) {FactoryBot.create(:family_member, family: test_family).person}
      let(:parent1) {FactoryBot.create(:family_member, family: test_family).person}
