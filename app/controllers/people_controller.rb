    ...
    if @dependent.blank?
      ...
      respond_to do |format|
        if member.save && @dependent.save
          # @person.ensure_relationship_with(member, params[:family_member][:primary_relationship], family.id)
          @dependent.person.add_relationship(@family.primary_applicant.person, params[:family_member][:primary_relationship], family.id, true)
          @family.primary_applicant.person.add_relationship(@dependent.person, PersonRelationship::InverseMap[params[:family_member][:primary_relationship]], family.id)
